...
const ControlsSection = (): ReactElement => {
    ...
    const handlePlayButtonClick = () => {
        if (isPlaying)
            dispatch(pauseSong())
        else if (selectedSongTitle !== '')
            dispatch(playSong(selectedSongTitle))
    }
    ...
    const handlePreviousSongClick = () => {
        if (selectedSongTitle !== '') {
            ...
            if (!isRepeat) { // if repeat is on go to the same song
                if (id == 0) id = length - 1 // if its the first song in album go to the last one
            ...
            } else { // if repeat is on go to the beginning of the song
    ...
    const handleNextSongClick = () => {
        if (selectedSongTitle !== '') {
            const { length } = selectedAlbumSongs // number of songs in selected album
            let id: number = parseInt(selectedSongTitle.split(' ')[0]) - 1 // id is first word in every title
            if (!isRepeat) { // if repeat is off change song
                if (id == length - 1) id = 0 // if its the last song in album go to the first one
                else id++ // if it isnt the last one go to next
                if (isShuffle) id = Math.floor(Math.random() * length) // if shuffle is on get random one
                dispatch(changeSong(id))
            } else { // if repeat is on go to the beginning
