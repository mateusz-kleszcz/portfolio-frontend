...
const SongLabel = ({ songId, name, album, duration }: SongLabelProps) => {
    ...
    const { selectedSongTitle, isPlaying } = useSelector((state: AppState) => state.playerReducer)
    ...
    useEffect(() => {
        const newName = name.split('.').slice(0, -1).join().split(' ').slice(1).join(' ')
    ...
    useEffect(() => {
        ...
        else if (selectedSongTitle === name)
